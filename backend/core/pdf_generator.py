...
class AnswerSheetGenerator:
    def __init__(self, output_path, layout_path, exam_id):
        ...
        # Dimensiones A4 Landscape: 841.89 x 595.28 pts
        ...
        # Contrato: Coordenadas guardadas en origen TOP-LEFT (0,0) para el escáner
        self.layout = {
    ...
    def draw_identification_side(self, c):
        ...
        for col in range(8):
            ...
            for row in range(10):
                ...
                self.layout["dni_bubbles"][col_key][str(row)] = [bx, by] # GUARDAR TOP-DOWN
        ...
        for i, t in enumerate(["P", "Q", "R", "S", "T"]):
            ...
            self.layout["exam_type_bubbles"][t] = [bx, by] # GUARDAR TOP-DOWN
        ...
        for i, label in enumerate(labels):
            ...
            self.layout["name_field_bbox"][label] = [start_x, y - 24, box_w, box_h] # GUARDAR TOP-DOWN
        ...
        for block in range(2):
            ...
            for q_idx in range(30):
                ...
                for i, opt in enumerate(["A", "B", "C", "D", "E"]):
                    bx, by = bx_offset + 35 + (i * 26), y + 3
                    rbx, rby = self.pos(bx, by)
                    c.circle(rbx, rby, 5.5, stroke=1, fill=0)
                    c.setFont("Helvetica-Bold", 7)
                    c.drawCentredString(rbx, rby - 2, opt)
                    self.layout["answer_bubbles"][str(q_num)][opt] = [bx, by] # GUARDAR TOP-DOWN
